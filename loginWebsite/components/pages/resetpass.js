...
class resetpass extends React.Component {
    ...
    async checkIfAuthorized() {
        const res = await fetch(`/authweb/user/checkresettoken/${this.props.location.search}`)
        if (res.status === 200) {
            this.setState({ authorized: true })
        } else {
            this.props.history.push('/authweb/permissiondenied')
        }
    }
    async doReset() {
        ...
        const res = await fetch('/authweb/user/resetpass', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: this.props.location.search.split('=')[1], password: pass }) })
        if (res.status === 200) {
            this.setState({ resetComplete: true })
        }
    }
    backtoHome() {
        this.props.history.push('/authweb')
    }
    render() {
