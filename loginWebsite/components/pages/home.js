...
class home extends React.Component {
  ...
  async checkForlogin() {
    const res = await fetch('/authweb/auth/checkaccess', { method: 'GET', headers: { 'Access-Control-Expose-Headers': 'token', 'auth-token': this.state.authToken } })
    ...
    if (res.status === 200) {
      ...
      this.props.history.replace('/authweb', this.props.history)
      this.props.history.push('/authweb/dashboard', { email: this.state.email })
    }
  }
  async jwtDecode() {
    let token = {};
  ...
  async checkAuth() {
    if (this.state.loggedin === true) {
      ...
      this.setState({ authToken: info.token })
      if (res.status1 === 405) {
        this.history.push('/authweb/logout')
      }
  ...
  async dosignin(e) {
    ...
    const res = await fetch('/authweb/user/login',
      {
        method: 'POST',
    ...
    if (res.status === 200) {
      ...
      this.props.history.push('/authweb/dashboard')
    } else {
      const info = await res.json()
      console.log(info)
  ...
  async dosignup(e) {
    ...
    const res = await fetch('/authweb/user/register', { method: 'post', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name, email: email, password: pass1 }) })
    if (res.status === 200) {
      this.setState({ loggedin: true })
      this.setState({ email: email })
      this.props.history.push('/authweb/dashboard')
  ...
  async doForgotPass() {
    ...
    const res = await fetch('/authweb/user/resetpasstoken', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email }) })
    if (res.status !== 200) {
